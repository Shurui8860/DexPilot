...
class SeqRetargeting:
    def __init__(
        ...
        """
    Wraps an Optimizer to perform sequential retargeting of human‐hand data
    into robot joint commands, optionally enforcing joint limits and
    applying a low‐pass filter.
        ...
        # Create a [n_joints × 2] array of [min, max] limits, defaulting to "no limit"
        joint_limits[:, 0] = -1e4  # a large value is equivalent to no limit
        joint_limits[:, 1] = 1e4

        if has_joint_limits:
            # Replace with actual robot limits if enabled
            joint_limits[:] = robot.joint_limits[:]
            # Tell the optimizer which subset of those limits apply to target joints
    ...
    def retarget(self, ref_value, fixed_qpos=np.array([])):
        """
        Run one optimization step to map human reference values to robot qpos.
        Optionally, some DoFs can be held fixed by passing fixed_qpos.
        """
        ...
        # Store current solution for next iteration’s warm start
        self.last_qpos = qpos

        # Build full robot qpos vector, inserting fixed and optimized DoFs
        ...
        # If mimic‐joint adaptor exists, apply it
    ...
    def set_initial_qpos(self, initial_xyz):
        # Take first 3 values only
        xyz = np.asarray(initial_xyz[0], dtype=self.last_qpos.dtype)

        # Copy last_qpos and overwrite first three entries
        q = np.array(self.last_qpos, copy=True)
        q[:3] = xyz

        self.last_qpos = q

    def set_bounds(self, dt: Optional[float] = 1/15):
        """
        Update optimizer bounds for the current frame.

        - Always applies static joint limits.
        - If dt is provided, also applies velocity-based bounds around last_qpos.
        """
        self.optimizer.set_bounds(
            self.joint_limits,
            dt=dt,
            last_qpos=self.last_qpos if dt is not None else None,
        )
