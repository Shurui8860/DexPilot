...
class RetargetingConfig:
    ...
    def load_from_file(
        ...
        with path.open("r") as f:
            ...
            # Convert relative URDF path to absolute path based on YAML location
            if "urdf_path" in cfg:
                urdf_path = Path(cfg["urdf_path"])
                if not urdf_path.is_absolute():
                    # Make URDF path absolute relative to YAML file location
                    cfg["urdf_path"] = str((path.parent / urdf_path).resolve())
