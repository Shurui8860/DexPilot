...
class Optimizer:
    ...
    def __init__(
        ...
        # self.opt = nlopt.opt(nlopt.LD_LBFGS, len(idx_pin2target))
    ...
    def set_bounds(
        ...
        """
        Set optimization bounds for the *optimized* DOFs (idx_pin2target).

        - Always requires `joint_limits` of shape (opt_dof, 2). These are the static limits.
        - If `dt` and `last_qpos` are provided, further tightens bounds using a per-frame
          velocity window based on the robot's velocity limits:
             [last_qpos - vlim*dt, last_qpos + vlim*dt],
          then intersects with the static limits.
        - Stores static limits so future calls can also intersect against them.

        Args:
            joint_limits:  (opt_dof, 2) array of [lower, upper] for optimized joints.
            epsilon:       Small padding applied to static limits (expand by ±epsilon).
            dt:            Frame time (sec). If provided, enables velocity window.
            last_qpos:     Previous solution (shape (opt_dof,)) required when dt is set.
            vel_epsilon:   Numerical gap to ensure lower < upper after intersection.
        """
        # --- validate static limits ---
        if joint_limits.shape != (self.opt_dof, 2):
            raise ValueError(
                f"Expect joint limits have shape {(self.opt_dof, 2)}, got {joint_limits.shape}"
            )
        static_lower = (joint_limits[:, 0] - epsilon).astype(float)
        static_upper = (joint_limits[:, 1] + epsilon).astype(float)
        if np.any(static_lower > static_upper):
            raise ValueError("Invalid joint limits: lower bound exceeds upper bound.")

        # # cache static limits for possible later use
        # self._static_lower = static_lower.copy()
        # self._static_upper = static_upper.copy()

        # default bounds = static
        lower = static_lower
        upper = static_upper

        # --- optional velocity window ---
        if dt is not None or last_qpos is not None:
            if dt is None or dt <= 0:
                raise ValueError("dt should be a positive number when using velocity bounds.")
            if last_qpos is None:
                raise ValueError("last_qpos must be provided when using velocity bounds.")
            last_qpos = np.asarray(last_qpos, dtype=float)
            if last_qpos.shape != (self.opt_dof,):
                raise ValueError(f"last_qpos must have shape ({self.opt_dof},), got {last_qpos.shape}")

            # pull full robot velocity limits and slice to optimized DOFs
            full_vlim = self.robot.get_velocity_limits()  # shape (nq,)
            vlim_opt = np.asarray(full_vlim, dtype=float)[self.idx_pin2target]  # (opt_dof,)

            step = vlim_opt * float(dt)  # inf allowed, yields open window
            dyn_lower = last_qpos - step
            dyn_upper = last_qpos + step

            # intersect with static limits
            lower = np.maximum(static_lower, dyn_lower)
            upper = np.minimum(static_upper, dyn_upper)

            # ensure valid interval numerically
            lower = np.minimum(lower, upper - vel_epsilon)
    ...
    def retarget(self, ref_value, fixed_qpos, last_qpos):
        ...
        try:
            lower_bounds = self.opt.get_lower_bounds()
            upper_bounds = self.opt.get_upper_bounds()
            initial_guesses = [
                lower_bounds,
                upper_bounds,
                (lower_bounds + upper_bounds) / 2,
                np.clip(np.array(last_qpos).astype(np.float32), lower_bounds, upper_bounds),
            ]
            def optimize(initial_guess):
                sol = self.opt.optimize(initial_guess)
                val = self.opt.last_optimum_value()
                return sol, val
            solutions = [optimize(guess) for guess in initial_guesses]
            qpos, _ = min(solutions, key=lambda x: x[1])
...
class DexPilotOptimizer(Optimizer):
    ...
    def __init__(
        ...
        super().__init__(
            robot, wrist_link_name, target_joint_names
        )
        self.target_link_human_indices = target_link_human_indices
        self.origin_link_names = target_origin_link_names
        self.task_link_names = target_task_link_names
